...
const blockContentSchema = defaultSchema
  .get('page')
  .fields.find((field: FieldDefinition) => field.name === 'excerpt').type

// https://github.com/sanity-io/sanity/blob/next/packages/%40sanity/block-tools/README.md
export async function htmlToBlockPageExcerpt(
  html: string,
  client: SanityClient,
  imageCache: Record<number, string>,
): Promise<Page['excerpt']> {
  ...
  const blocksWithUploads = blocks.map((block) =>
    limit(async () => {
      ...
      if (imageCache[imageId]) {
        ...
        } as Extract<Page['excerpt'], {_type: 'image'}>
      }
